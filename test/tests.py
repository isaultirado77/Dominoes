...
class TestStage3(StageTest):
    ...
    def fill_the_snake(self, output, flag=False):
        ...
        if flag:
            ...
            snake = self.parse_the_output(output.strip())[3]
        try:
            left = eval(snake[:6])
            right = eval(snake[-6:])
    ...
    def check_player_unique(self, output):
        """Check that the player pieces are unique"""
    ...
    def get_the_ends(self, output):
        ...
        try:
            if 'invalid input' in output.lower():
                raise WrongAnswer("When 0 is entered, the player should take an extra piece from the stock")
    ...
    def check_the_move(self, output):
        ...
        if not self.check_the_status(output):
            raise WrongAnswer("The status is not right. "
