...
class TestStage4(StageTest):
    ...
    def get_the_ends(self, output):
        ...
        try:
            if 'invalid input' in output.lower():
                raise WrongAnswerException("When 0 is entered, the player should take an extra piece from the stock")
    ...
    def check_the_piece(self, output):
        ...
        domino_snake = self.parse_the_output(output.strip())[3]
        try:
    ...
    def check_the_move(self, output, to_fail=False, mistake=None):
        ...
        if not self.check_the_status(output):
            raise WrongAnswerException("The status is not right. "
                              "Please note that the status should indicate which player is to make the next move.\n"
                              "This error can also occur when 0 is entered and the player doesn't take an extra piece from the stock.")
    ...
    def func4(self, output):
        if self.current_status == 'computer':
            if "illegal move" not in output.lower() and "please try again" not in output.lower():                
    ...
    def func7(self, output):
        if self.current_status == 'computer':
            if "invalid input" not in output.lower() and "please try again" not in output.lower():
                return CheckResult.wrong("The player should be informed about tne incorrect move")
            return self.chosen_piece
        else:
            self.set_the_currents(output)
    ...
    def check_the_win(self, reply: list, attach: Any) -> CheckResult:
        ...
        if '1:[' not in last_output:
            ...
            if "the game is over. you won" not in the_last[-1].lower():
                return CheckResult.wrong("The status is not right.\n"
                                     "Expected: \"The game is over. You won.\"")
        # check for the computer win
            ...
            if "the game is over. the computer won" not in the_last[-1].lower():
                return CheckResult.wrong("The status is not right.\n"
                                     "Expected: \"The game is over. The computer won.\"")
        else:
